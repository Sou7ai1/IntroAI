...
def main():
    ...
    test_cases_challenge = [
        (range_except(100, [1,2,7,54,96]), 96, [4, 6, 8, 12, 16, 24, 32, 48]),
        (range_except(100, [2,7,54,96]), 96, [4, 6, 8, 12, 16, 24, 32, 48]),
        (range_except(100, [2,4,6,7,8,12,16,24,32,48,54,96]), 96, []),
        (range_except(150, [4,6,8,9,100]), 100, [10, 20, 50]),
        (range_except(150, [4,6,8,9,10,20,50,100]), 100, []),
    ]

    tests = {
            "trivial": (test_cases_trivial, 2, 60),
            "small": (test_cases_small, 2, 60),
            "medium": (test_cases_medium, 3, 60),
            "large": (test_cases_large, 3, 60),
            "challenge": (test_cases_challenge, 3, 60),
    }

    if len(sys.argv) == 1:
        results = PrettyTable(["Test name", "Points", "Time limit on recodex [s]", "Your time [s]", "Evaluation"])
        for name in tests:
