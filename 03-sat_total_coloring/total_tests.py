...
def main():
    small_graphs = [
        ...
        ("Barbell graph of size 3", networkx.barbell_graph(3,1), 4),
        ("Wheel graph of size 4", networkx.wheel_graph(4), 5),
        ("3-regular Platonic Tetrahedral graph", networkx.tetrahedral_graph(), 5)
    ]
    large_graphs = [
        ...
        ("Barbell graph of size 6", networkx.barbell_graph(6,6), 7),
        ("Les Miserables", networkx.les_miserables_graph(), 37)
    ]

    tests = {
            "small": (small_graphs, 5, 60),
            "large": (large_graphs, 5, 60)
    }
