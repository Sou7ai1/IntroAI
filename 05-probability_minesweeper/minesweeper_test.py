...
def main():
    ...
    tests = {
        "first": (5, 7, 0.2, 4, 100, [31,39], 60), # 28, 42
        "second": (9, 8, 0.1, 6, 100, [70,76], 60), # 63, 80
        "third": (12, 15, 0.15, 42, 100, [55,68], 60), # 42, 72
        "fourth": (15, 18, 0.16, 123, 50, [21,30], 60), # 17, 33
        "fifth": (30, 25, 0.12, 4125432, 30, [19,24], 60), # 16, 26
    }

    if len(sys.argv) == 1:
        results = PrettyTable(["Test name", "Rows", "Columns", "Mine probability", "Games", "Won", "Points", "Time limit on recodex [s]", "Limit on Recodex [s]"])
        for name in tests:
            ...
            print()
            results.add_row([name, rows, columns, mine_prb, games, won, points, running_time, time_limit])
        print(results)
    else:
        name = sys.argv[1]
        if name in tests:
